...
export class CandidatesService {
  ...
  async registerCandidate(
    ...
    if (existing)
      throw new BadRequestException(
        'This user is already registered as a candidate.',
      );
  ...
  async addMaterial(
    ...
    if (payload.type !== 'manifesto' && !payload.link)
      throw new BadRequestException(
        'A link is required for this material type.',
      );
    ...
    switch (payload.type) {
      case 'manifesto':
        if (!payload.manifestos || payload.manifestos.length === 0)
          throw new BadRequestException(
            'Manifesto requires at least one entry.',
          );
        ...
        payload.manifestos.forEach((m) => {
          if (!m.title || !m.description)
            throw new BadRequestException(
              'Each manifesto requires a title and description.',
            );
        });
      ...
      case 'video':
        if (!payload.title || !payload.description)
          throw new BadRequestException(
            'Video requires a title and description.',
          );
      ...
      case 'slide':
        if (!payload.title)
          throw new BadRequestException('Slide requires a title.');
        material = await prisma.slide.create({
          data: {
            candidateId,
            slideTitle: payload.title,
            slideLink: payload.link!,
          },
        });
        break;
      ...
      default:
        throw new BadRequestException(
          'Invalid material type. Must be manifesto, video, slide, or poster.',
        );
    }

    await this.auditLogsService.logAction(
      superAdminId,
      'ADDED_CANDIDATE_MATERIAL',
      {
        candidateId,
        type: payload.type,
        materialCount:
          payload.type === 'manifesto' ? payload.manifestos?.length : 1,
      },
    );
  ...
  async upsertQualification(
    ...
    await this.auditLogsService.logAction(
      superAdminId,
      'UPSERTED_CANDIDATE_QUALIFICATION',
      {
        candidateId,
  ...
  async updateCandidate(
    ...
    await this.auditLogsService.logAction(superAdminId, 'UPDATED_CANDIDATE', {
      candidateId,
      updates,
    });
    return updated;
  ...
  async deleteCandidate(candidateId: string, superAdminId: string) {
    ...
    await this.auditLogsService.logAction(superAdminId, 'DELETED_CANDIDATE', {
      candidateId,
    });
  ...
  async updateMaterial(
    ...
    switch (type) {
      case 'manifesto':
        material = await prisma.manifesto.update({
          where: { id: materialId },
          data: { title: payload.title, description: payload.description },
        });
        break;
      case 'video':
        material = await prisma.video.update({
          where: { id: materialId },
          data: {
            videoTitle: payload.title,
            videoDescription: payload.description,
        ...
        break;
      case 'slide':
        material = await prisma.slide.update({
          where: { id: materialId },
    ...
    await this.auditLogsService.logAction(superAdminId, 'UPDATED_MATERIAL', {
      candidateId,
      type,
      materialId,
    });
  ...
  async deleteMaterial(
    ...
    await this.auditLogsService.logAction(superAdminId, 'DELETED_MATERIAL', {
      candidateId,
      type,
      materialId,
    });
